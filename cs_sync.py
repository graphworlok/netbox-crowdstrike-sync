...
def _load_token_file(path: Optional[str]) -> dict:
    """Load and validate the CS_FEM_TOKEN JSON file. Exits on any error."""
    ...
    try:
        ...
        _ = creds["client_id"], creds["client_secret"]   # validate required keys
    except json.JSONDecodeError as exc:
        console.print(f"[red]CS_FEM_TOKEN is invalid JSON:[/red] {exc}")
        sys.exit(1)
    except KeyError as exc:
        console.print(f"[red]CS_FEM_TOKEN missing required key:[/red] {exc}")
        sys.exit(1)
    return creds
